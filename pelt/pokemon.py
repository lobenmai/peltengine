...
class PlayerData: # XXX not entirely fixed
	...
	def load_data(self, dom): #load data from a dom
		...
		d.gender_ratio = bool(g(dom, "gender"))
		#load type data
		d.type = [x for x in g(dom, "type").split("|")]
		#load pokedex data
		t = data.get_node(dom, "physattr")
		d.physattr = Container()
		d = d.physattr
...
def load_data(): #load all pokemon data
	...
	pokemen = data.load_xml("pokemon_data.xml", peltdir=False).documentElement # get list of pokemon
	for pokemon in pokemen.getElementsByTagName("pokemon"): #loop through all pokemon
		...
		poke_data = data.load_xml(data.get_node_text(pokemon), peltdir=False).documentElement #load data file
		pokemon_data[name] = PokemonData(poke_data) #load and parse data
	nature_data = {} #clear out nature data
	natures = data.load_xml("nature.xml", peltdir=False).documentElement #get nature data
